...
for branch in $TARGET_BRANCH REL_13_STABLE REL9_6_STABLE ; do
  ...
  if echo $branch | grep -v -E 'REL[6789]' > /dev/null ; then
    ...
    install-icu/$branch/bin/psql postgres -c 'create table c (t text collate "en_US")'
    ...
    install-icu/$branch/bin/psql postgres -c 'create table e (t text collate "fr-x-icu")'
    install-icu/$branch/bin/psql postgres -c 'create index on e(t)'
